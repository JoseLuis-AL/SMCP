...
/// <summary>
/// Constructs a PixmapQWidget with an empty display and an invalidated scale cache.
/// </summary>
/// <param name="parent">Parent widget (passed to QWidget).</param>
/// <param name="flags">Window flags (passed to QWidget).</param>
SMCP::PixmapQWidget::PixmapQWidget(QWidget* parent, Qt::WindowFlags flags)
...
void SMCP::PixmapQWidget::setImage(const cv::Mat& image)
{
...
void SMCP::PixmapQWidget::setImage(const QPixmap& pixmap)
{
	{
		QMutexLocker locker(&mutex);
...
void SMCP::PixmapQWidget::setPixmap(const QPixmap& pixmap)
{
	setImage(pixmap);
}

const QPixmap* SMCP::PixmapQWidget::pixmap() const
{
	return &current_pixmap;
}

void SMCP::PixmapQWidget::clear()
{
	{
		QMutexLocker locker(&mutex);
...
void SMCP::PixmapQWidget::paintEvent(QPaintEvent* event)
{
	Q_UNUSED(event);
...
void SMCP::PixmapQWidget::resizeEvent(QResizeEvent* event)
{
...
QPixmap SMCP::PixmapQWidget::pixmapFromBGR(const cv::Mat& bgr)
{
	if (bgr.type() != CV_8UC3)
	{
		return {};
...
QPixmap SMCP::PixmapQWidget::pixmapFromGray(const cv::Mat& gray)
{
...
void SMCP::PixmapQWidget::invalidateCache()
{
	cache_valid = false;
}
